...
class HueController:
    def __init__(self, bridge_ip: str = None, username: str = None):
        """
        ...
        """
        bridge_ip = bridge_ip or os.getenv("HUE_BRIDGE_IP")
        username = username or os.getenv("HUE_USERNAME")
        ...
        try:
            ...
            self.bridge.connect()  # Safe if already registered
            self.lights = self.bridge.get_light_objects('name')
            self.available = True
            print("[HUE] Connected to bridge")

        except PhueRequestTimeout:
            print("[HUE] Bridge not reachable, Hue disabled")

        except Exception as e:
            print(f"[HUE] Unexpected error during init: {e}")
